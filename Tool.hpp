...
class Tool
{
    ...
    public:
        Tool (std::vector<double> coeff, double eps = 1e-14)
        {
            while (true)
            {
                if (coeff.size() == 0) break;

                if (abs (coeff.back()) < eps) coeff.pop_back();
                else break;
            }
        ...
        int deg() const { return static_cast<int> (coeff_.size()) - 1; }
        ...
        double integrate (bool enable_k)
        {
            ...
            else if (end_ - start_ == 1)
            {
                ...
                {        // N_ is odd
                    ...
                    else return tool_.integrate (a, c);
                }
            }
